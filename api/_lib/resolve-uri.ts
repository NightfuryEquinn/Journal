...
/** Bun/Node on Windows often fails `dns.resolveSrv` for Atlas — fall back to nslookup. */
...
/** Resolve Atlas SRV records via Node DNS. */
...
/** Build a direct mongodb:// URI for one shard host. */
...
/** Probe SRV hosts and return a URI that points at the writable primary. */
 ...
 * Only needed on Windows where DNS cannot resolve Atlas SRV records.
...
export async function resolveMongoUri(uri: string): Promise<string> {
  ...
  const dbName = parsed.pathname.replace(/^\//, '') || process.env.MONGODB_DB || 'journs';
