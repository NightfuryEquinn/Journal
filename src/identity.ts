...
/** Compact BIP39-style demo wordlist (subset — not a real wallet seed). */
const WORDLIST = [
  ...
  'borrow', 'boss', 'both', 'bottle', 'bottom', 'bounce', 'box', 'boy',
  ...
  'item', 'ivory', 'jacket', 'jaguar', 'jar', 'jazz', 'jealous', 'jelly',
  ...
  'retire', 'retreat', 'return', 'reveal', 'review', 'reward', 'rhythm', 'rib',
  ...
  'sms', 'snake', 'snap', 'sniff', 'snow', 'soap', 'soccer', 'social',
...
/** Demo hash — not security-grade; fine for local prototype only. */
export async function demoHash(input: string): Promise<string> {
  const data = new TextEncoder().encode(`journs.demo.v1:${input}`);
...
/** Derive a stable operator callsign from the recovery phrase. */
export async function deriveOperatorId(words: string[]): Promise<string> {
  const hash = await demoHash(phraseToString(words));
  return `op-${hash.slice(0, 8)}`;
}

/** Build a fake 0x address from operator id (simulated Web3 link). */
export async function fakeWalletAddress(operatorId: string): Promise<string> {
  const hash = await demoHash(`wallet:${operatorId}`);
  return `0x${hash.slice(0, 40)}`;
}

/** Shorten a wallet address for HUD display. */
...
/** Generate a random 12-word demo recovery phrase. */
export function generateRecoveryPhrase(): string[] {
  ...
  for (let i = 0; i < 12; i++) {
    out.push(WORDLIST[arr[i]! % WORDLIST.length]!);
  }

  return out;
}

/** Validate that words are exactly 12 entries from the demo wordlist. */
...
/** Persist device identity to localStorage. */
export function saveIdentity(identity: DeviceIdentity): void {
  localStorage.setItem(IDENTITY_KEY, JSON.stringify(identity));
}

/** Create a new identity from a recovery phrase and device passphrase. */
export async function createIdentity(
  words: string[],
  passphrase: string,
): Promise<DeviceIdentity> {
  const normalized = normalizePhrase(words);
  const err = validateRecoveryWords(normalized);
  ...
  const operatorId = await deriveOperatorId(normalized);
  const passphraseHash = await demoHash(passphrase.trim());
  const recoveryHash = await demoHash(phraseToString(normalized));
...
/** Verify device passphrase against stored hash. */
export async function verifyPassphrase(
  identity: DeviceIdentity,
  passphrase: string,
): Promise<boolean> {
  const hash = await demoHash(passphrase.trim());

  return hash === identity.passphraseHash;
}

/**
 * Recover on a new device: verify phrase (against stored hash if present,
 * or accept valid wordlist phrase when storage is empty), then set passphrase.
 */
export async function recoverWithPhrase(
  ...
  if (existing && existing.recoveryHash !== recoveryHash) {
    throw new Error('Recovery phrase does not match this device identity.');
  }

  const operatorId = existing?.operatorId ?? (await deriveOperatorId(normalized));
  const passphraseHash = await demoHash(newPassphrase.trim());
...
/** Connect simulated wallet; returns updated identity. */
export async function connectWallet(identity: DeviceIdentity): Promise<DeviceIdentity> {
  const walletAddress = await fakeWalletAddress(identity.operatorId);
  const next = { ...identity, walletAddress };
  saveIdentity(next);
...
export function disconnectWallet(identity: DeviceIdentity): DeviceIdentity {
  const next = { ...identity, walletAddress: null };
  saveIdentity(next);
