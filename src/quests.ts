...
export const PROGRESS_KEY = 'journs.progress.v1';
...
/** ISO-week key YYYY-Www in local time. */
export function weekKey(d: Date = new Date()): string {
  const tmp = new Date(d.getFullYear(), d.getMonth(), d.getDate());
  const day = (tmp.getDay() + 6) % 7;
  tmp.setDate(tmp.getDate() - day + 3);
  const first = new Date(tmp.getFullYear(), 0, 4);
  const week =
    1 +
    Math.round(
      ((tmp.getTime() - first.getTime()) / 86400000 - 3 + ((first.getDay() + 6) % 7)) / 7,
    );
...
/** True if entry date falls on the given local day key. */
function entryOnDay(entry: JournalEntry, dk: string): boolean {
  return dayKey(new Date(entry.date)) === dk;
}

/** True if entry date falls in the given ISO week key. */
...
/** Progress fraction 0–1 for display bars. */
 ...
 * Call on unlock and when opening profile.
 */
