...
export function ListScreen({
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 pt-4 pb-6 max-phone:px-3 laptop:px-7 laptop:pt-5 laptop:pb-7">
      <div className="mb-[22px] grid grid-cols-1 items-center gap-3 max-tablet:gap-3 tablet:grid-cols-[1fr_auto_auto] tablet:gap-[18px]">
        <Bracket className="min-w-0">
        ...
        <div className="inline-flex flex-wrap items-center gap-1.5 border border-line p-1">
          <span className="font-display text-[9px] font-medium tracking-[0.12em] text-fg-mute uppercase">
            LAYOUT
          </span>
          {(['timeline', 'stack'] as const).map((k) => (
            <button
              key={k}
              type="button"
              className={`tap-target px-3 py-1.5 font-display text-[10px] tracking-[0.18em] max-tablet:min-h-11 ${
        ...
        <Btn variant="primary" onClick={onNew} className="max-tablet:w-full tablet:w-auto">
          + NEW ENTRY
        </Btn>
      </div>

      <div className="grid grid-cols-1 items-start gap-5 tablet:grid-cols-[minmax(0,280px)_1fr] tablet:gap-[22px]">
        <aside className="min-w-0">
          ...
          <Panel title="TAGS" meta={`${allTags.length} total`}>
            <div className="flex flex-col gap-1">
              <button
                type="button"
                className={`tap-target grid grid-cols-[14px_1fr_auto] items-center gap-2 border px-2.5 py-[7px] text-left font-mono text-[11px] tracking-[0.06em] max-tablet:min-h-11 ${
                  !tagFilter
                    ? 'border-accent bg-accent-soft text-accent'
                    : 'border-transparent bg-black/20 text-fg-dim hover:bg-white/[0.03] hover:text-fg'
                }`}
                onClick={() => {
                  setTagFilter(null);
                  SoundManager.click();
        ...
        <main className="min-w-0">
          ...
          {filtered.length === 0 && (
            <div className="py-[60px] text-center font-mono tracking-[0.02em] text-fg-dim">
...
function TimelineLayout({ entries, onOpen, onDelete }: TimelineLayoutProps) {
  return (
    <div className="relative flex flex-col gap-3.5 pl-0 max-phone:pl-0 tablet:pl-20">
      <div className="absolute top-1.5 bottom-1.5 left-[92px] w-px bg-line-strong max-tablet:hidden" />
      {entries.map((e, i) => {
        ...
        return (
          ...
          >
            <div className="flex items-baseline gap-2 pt-0 text-left leading-[1.1] tablet:-ml-20 tablet:block tablet:pr-3.5 tablet:pt-[18px] tablet:text-right">
            ...
            <div className="relative w-[18px] max-tablet:hidden">
              <span
                className="absolute top-[22px] left-2 size-2.5 rotate-45 bg-accent shadow-[0_0_12px_var(--accent)]"
                aria-hidden="true"
              />
            </div>
            <div
              className="group relative ml-0 cursor-pointer border border-line-strong bg-[linear-gradient(180deg,rgba(255,255,255,0.012),rgba(0,0,0,0.18)),var(--bg-1)] p-3.5 pr-4 transition-[transform,border-color,box-shadow] duration-[180ms] [clip-path:polygon(0_0,calc(100%-16px)_0,100%_16px,100%_100%,16px_100%,0_calc(100%-16px))] hover:translate-x-0.5 hover:-translate-y-0.5 hover:border-accent tablet:ml-3"
              style={{
                boxShadow:
            ...
            >
              <span
                className="absolute top-[22px] -left-3 h-px w-3 bg-line-strong max-tablet:hidden"
                aria-hidden="true"
              />
              ...
              <button
                type="button"
                className="tap-target absolute top-2.5 right-2.5 size-[22px] text-[11px] text-fg-mute opacity-0 transition-[opacity,color] duration-150 group-hover:opacity-100 hover:text-bad max-tablet:min-h-11 max-tablet:min-w-11 max-tablet:opacity-100"
                onClick={(ev) => {
                  ev.stopPropagation();
                  SoundManager.click();
                  onDelete(e);
      ...
      {entries.map((e, i) => {
        ...
        return (
          ...
          >
            ...
            <div className="line-clamp-4 flex-1 text-xs leading-[1.5] text-fg-dim">
              {e.body.split('\n').slice(0, 3).join(' ')}
            </div>
